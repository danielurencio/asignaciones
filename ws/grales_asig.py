...
class Service():
    def __init__(self):
        self.connStr = h#'mssql+pyodbc://daniel.urencio:Hola12345@UMBRACOSQL03\\CNHSQLU03:49730/estadisticas?driver=SQL+Server'
...
#                            "SELECT A.*,B.TIPO,B.CUENCA,B.UBICACION FROM CMDE_PUBLIC." + usr + "ASIGNACIONES_PRODUCCION A " +\
#                            "LEFT JOIN CMDE_RAW." + usr + "ASIGNACIONES_GRALES B " +\
#                            "ON A.ID = B.ID " +\
#                           ") " + self.conditionalQuery() +\
#                           "GROUP BY FECHA",

            'produccion': u"SELECT FECHA,SUM(ACEITE_MBD) AS ACEITE_MBD, SUM(GAS_MMPCD) AS GAS_MMPCD FROM " + usr + "ASIGNACIONES_PRODUCCION " +\
	                  "WHERE ID IN ( " +\
			  "  SELECT DISTINCT ID FROM " + usr + "ASIGNACIONES_GRALES " + self.conditionalQuery() +\
			  ") GROUP BY FECHA",

#            'pozos_inv_': "SELECT FECHA,DESCRIPTOR,SUM(VALOR) AS VALOR FROM ( " +\
#                            "SELECT A.*,B.CUENCA,B.UBICACION,B.TIPO FROM " + usr + "ASIGNACIONES_INVPOZOS A " +\
#                            "LEFT JOIN " + usr + "ASIGNACIONES_GRALES B " +\
#                            "ON A.ID = B.ID " +\
#                            "ORDER BY FECHA " +\
#                          ") " + self.conditionalQuery() +\
#                          "GROUP BY FECHA,DESCRIPTOR",
            ...
            'pozos_inv': "SELECT * FROM (" +\
	                 "SELECT FECHA,DESCRIPTOR,SUM(VALOR) AS VALOR FROM " + usr + "ASIGNACIONES_INVPOZOS " +\
                         "WHERE ID IN ( " +\
                         "  SELECT DISTINCT ID FROM " + usr + "ASIGNACIONES_GRALES " + self.conditionalQuery() +\
                         ") GROUP BY FECHA,DESCRIPTOR" +\
            ...
            'reservas_': "SELECT FECHA,TIPO_RESERVAS AS TIPO, SUM(RR_PCE_MMBPCE) AS RR_PCE_MMBPCE, SUM(RR_ACEITE_MMB) AS RR_ACEITE_MMB, SUM(RR_GAS_NATURAL_MMMPC) AS RR_GAS_NATURAL_MMMPC from ( " +\
                           "SELECT A.*, B.CUENCA, B.TIPO, B.UBICACION FROM ( " +\
                             ...
                             "FROM "+ usr +"RESERVAS_ASIG_CONT_TEST WHERE ASIGNACION_CONTRATO LIKE 'A%' GROUP BY FECHA,TIPO,NOMBRE_ASIG,ID_ASIG " +\
                             "ORDER BY FECHA,NOMBRE_ASIG,TIPO " +\
                           ") A LEFT JOIN CMDE_RAW." + usr + "ASIGNACIONES_GRALES B " +\
                           "ON A.ID = B.ID " +\
                         ") " + self.conditionalQuery() +\
                         "GROUP BY FECHA,TIPO_RESERVAS",

            'reservas': "SELECT FECHA,TIPO,SUM(RR_PCE_MMBPCE) AS RR_PCE_MMBPCE, SUM(RR_ACEITE_MMB) AS RR_ACEITE_MMB, SUM(RR_GAS_NATURAL_MMMPC) AS RR_GAS_NATURAL_MMMPC FROM "+ usr +"RESERVAS_ASIG_CONT_TEST " +\
                        "WHERE ID_ASIG IN ( " +\
                          " SELECT DISTINCT ID FROM " + usr + "ASIGNACIONES_GRALES " + self.conditionalQuery() +\
                        ") GROUP BY FECHA,TIPO ORDER BY FECHA,TIPO",

            'cmt_ext_': "SELECT ANIO,CONCEPTO,SUM(VALOR) AS VALOR FROM ( " +\
                            "SELECT A.*, B.CUENCA, B.UBICACION, B.TIPO FROM (SELECT * FROM " + usr + "ASIGNACIONES_CMT_EXTRACCION WHERE CONCEPTO NOT IN ('Qo','Qg')) A " +\
                            "LEFT JOIN " + usr + "ASIGNACIONES_GRALES B " +\
                            "ON A.ID = B.ID " +\
                        ") " + self.conditionalQuery() +\
            ...
            'cmt_ext': "SELECT ANIO,CONCEPTO,SUM(VALOR) AS VALOR FROM " + usr + "ASIGNACIONES_CMT_EXTRACCION WHERE CONCEPTO NOT IN ('Qo','Qg') AND " +\
                       " ID IN ( SELECT DISTINCT ID FROM " + usr + "ASIGNACIONES_GRALES "+ self.conditionalQuery() +") "+\
                       "GROUP BY ANIO,CONCEPTO ORDER BY ANIO,CONCEPTO",

            'cmt_exp_': "SELECT ANIO,CONCEPTO,SUM(VALOR) AS VALOR FROM ( " +\
                            "SELECT A.*, B.CUENCA, B.UBICACION, B.TIPO FROM " + usr + "ASIGNACIONES_CMT_EXPLORACION A " +\
                            "LEFT JOIN " + usr + "ASIGNACIONES_GRALES B " +\
                            "ON A.ID = B.ID " +\
                       ") " + self.conditionalQuery() +\
                       "GROUP BY ANIO,CONCEPTO " +\
            ...
            'cmt_exp':"SELECT ANIO,CONCEPTO,SUM(VALOR) AS VALOR FROM " + usr + "ASIGNACIONES_CMT_EXPLORACION " +\
                        "WHERE ID IN ( SELECT DISTINCT ID FROM " + usr + "ASIGNACIONES_GRALES "+ self.conditionalQuery() +" ) "+\
                        "GROUP BY ANIO,CONCEPTO ORDER BY ANIO,CONCEPTO",

            'seguimiento_ext_': "SELECT ANIO,TIPO_OBSERVACION,CONCEPTO,SUM(VALOR) AS VALOR FROM ( " +\
                                "SELECT A.*, B.CUENCA, B.UBICACION, B.TIPO FROM ("+\
                                    "SELECT * FROM " + usr + "ASIGNACIONES_SEGUIMIENTO_EXT WHERE CONCEPTO NOT IN ('Np','Gp')" +\
                                    "ORDER BY " +\
                                      "CASE " +\
            ...
            'seguimiento_ext': "SELECT ANIO,TIPO_OBSERVACION,CONCEPTO,SUM(VALOR) AS VALOR FROM " + usr + "ASIGNACIONES_SEGUIMIENTO_EXT " +\
                               "WHERE CONCEPTO NOT IN ('Np','Gp') AND ID IN ( SELECT DISTINCT ID FROM " + usr + "ASIGNACIONES_GRALES "+ self.conditionalQuery() +") " +\
            ...
            'seguimiento_exp_': "SELECT ANIO,TIPO_OBSERVACION,CONCEPTO,SUM(VALOR) AS VALOR FROM ( " +\
                                "SELECT A.*, B.CUENCA, B.UBICACION, B.TIPO FROM " + usr + "ASIGNACIONES_SEGUIMIENTO_EXP_NEW A " +\
                                "LEFT JOIN " + usr + "ASIGNACIONES_GRALES B " +\
                                "ON A.ID = B.ID " +\
                            ") " + self.conditionalQuery() +\
                            "GROUP BY ANIO,TIPO_OBSERVACION,CONCEPTO " +\
            ...
            'seguimiento_exp': "SELECT ANIO,TIPO_OBSERVACION,CONCEPTO,SUM(VALOR) AS VALOR FROM " + usr + "ASIGNACIONES_SEGUIMIENTO_EXP "+\
                               "WHERE ID IN ( SELECT DISTINCT ID FROM " + usr + "ASIGNACIONES_GRALES "+ self.conditionalQuery() +") "+\
            ...
            'aprovechamiento_': "SELECT FECHA,VALOR FROM ( " +\
                                   "SELECT FECHA,CONCEPTO,SUM(VALOR) AS VALOR FROM ( " +\
                                        "SELECT A.*, B.CUENCA,B.TIPO,B.UBICACION FROM " + usr + "ASIGNACIONES_APROVECHAMIENTO A " +\
                                        "LEFT JOIN " + usr + "ASIGNACIONES_GRALES B " +\
                                        "ON A.ID = B.ID " +\
                                   ") " + self.conditionalQuery() +\
            ...
            'aprovechamiento': "SELECT FECHA,SUM(VALOR) AS VALOR FROM " + usr + "ASIGNACIONES_APROVECHAMIENTO "+\
    ...
    def catalogo_general(self):
        query = "SELECT * FROM " + usr + "ASIGNACIONES_GRALES WHERE VIGENTE = 1"
    ...
    def produccion(self):
        if(self.ID != 'Todas'):
            query = "SELECT * FROM " + usr + "ASIGNACIONES_PRODUCCION WHERE ID='" + self.ID + "'"
        else:
    ...
    def pozos_inv(self):
        if(self.ID != 'Todas'):
            query = "SELECT * FROM " + usr + "ASIGNACIONES_INVPOZOS WHERE ID='" + self.ID +"' "+\
    ...
    def reservas(self):
        if(self.ID != 'Todas'):
            query = "SELECT FECHA,TIPO,SUM(RR_PCE_MMBPCE) AS RR_PCE_MMBPCE,SUM(RR_ACEITE_MMB) AS RR_ACEITE_MMB, SUM(RR_GAS_NATURAL_MMMPC) AS RR_GAS_NATURAL_MMMPC FROM "+ usr +"RESERVAS_ASIG_CONT_TEST " +\
        ...
        def transform_reserves(x,col):
            ...
            for i,d in enumerate(fechas):
                # ---------------------------------------------------------------------------
                # Lo siguiente es necesario para que funcionen los WS en QA y en produccion.
                # ---------------------------------------------------------------------------
                if usr:
                    # QA:
    ...
    def cmt(self):
        ...
        if(self.ID != 'Todas'):
            ...
            if(tipo == 'AE'):
                query = "SELECT ANIO,CONCEPTO,VALOR FROM " + usr + "ASIGNACIONES_CMT_EXPLORACION WHERE ID='" + self.ID + "'"
                df = self.connectionResult(query,self.connStr)
                ...
                query = "SELECT ANIO,CONCEPTO,VALOR FROM " + usr + "ASIGNACIONES_CMT_EXTRACCION WHERE ID='" + self.ID + "' AND CONCEPTO NOT IN ('Qo','Qg')"
                df = self.connectionResult(query,self.connStr)
    ...
    def seguimiento(self):
        ...
        if(self.ID != 'Todas'):
            ...
            if(tipo == 'AE'):
                query = "SELECT ANIO,CONCEPTO,SUM(VALOR) AS VALOR,TIPO_OBSERVACION FROM " + usr + "ASIGNACIONES_SEGUIMIENTO_EXP WHERE ID='" + self.ID + "' GROUP BY ANIO,CONCEPTO,TIPO_OBSERVACION"
                df = self.connectionResult(query,self.connStr).to_json(orient='records')

            elif(tipo == 'A'):
                query1 = "SELECT ANIO,CONCEPTO,VALOR,TIPO_OBSERVACION FROM " + usr + "ASIGNACIONES_SEGUIMIENTO_EXT WHERE ID='" + self.ID + "' AND CONCEPTO NOT IN ('Np','Gp')"
                query2 = "SELECT ANIO,CONCEPTO,SUM(VALOR) AS VALOR,TIPO_OBSERVACION FROM " + usr + "ASIGNACIONES_SEGUIMIENTO_EXP WHERE ID='" + self.ID + "' GROUP BY ANIO,CONCEPTO,TIPO_OBSERVACION"
    ...
    def aprovechamiento(self):
        if(self.ID != 'Todas'):
            query = "SELECT * FROM " + usr + "ASIGNACIONES_APROVECHAMIENTO WHERE ID='" + self.ID + "' AND CONCEPTO = 'Gas natural No aprovechado  (mmpcd)'"
        else:
    ...
    def config_data(self):
        query = "select * from " + usr + "ASIGNACIONES_config"
